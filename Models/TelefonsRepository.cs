...
namespace WebServiceM9UF3.Models
{
    public class TelefonsRepository
    {
        ...
        {
            return RepositoryGlobal.dataContext.telefons.Where(x => x.telefon1.Contains(num) || x.tipus.Contains(num)).Select(x => x.contacte).Distinct().ToList().OrderBy(x => x.cognoms).ThenBy(x => x.nom).ToList();
        }
